...
def get_original_scrape_date(args):
    folder, job_id = args
    try:
        ...
        if lines and lines[0]:
            return job_id, datetime.strptime(lines[0], '%Y-%m-%d')
    except Exception as e:
        print(f"Failed to get git date for {job_id}: {e}")
    return job_id, datetime.now()
...
def fix_jobs(folder):
    ...
    # Pre-fetch scrape dates in parallel to be FAST!
    jobs_to_process = [j for j in jobs if j.get('matches_requirements') in ['yes', 'maybe'] and j.get('description_file')]
    args = [(folder, j['id']) for j in jobs_to_process]
    
    print(f"[{folder}] Fetching scrape dates for {len(args)} jobs...")
    scrape_dates = {}
    with ThreadPoolExecutor(max_workers=10) as executor:
        for job_id, ref_date in executor.map(get_original_scrape_date, args):
            scrape_dates[job_id] = ref_date
    ...
    for job in jobs_to_process:
        ...
        ref_date = scrape_dates.get(job['id'], datetime.now())
