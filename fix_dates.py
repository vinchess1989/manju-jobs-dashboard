...
def fix_jobs(folder):
    ...
    for job in jobs:
        if job.get('matches_requirements') in ['yes', 'maybe']:
            ...
            # Get the file creation time as reference
            ref_date = datetime.fromtimestamp(os.path.getmtime(desc_file))
            ...
            if len(parts) > 1:
                raw_text = parts[1][:1000].lower() # look at first 1000 chars for relative dates
                # Look for date patterns in the raw HTML text
                patterns = [
                    r'(\d+\s+(?:day|päivä|viikko|week|month|kuukaus|hour|tunti|min)s?\s+ago)',
                    r'(\d+\s*[dwm]\s*ago)',
                    r'(today|tänään|yesterday|eilen)'
                ]
                found_date_str = None
                for p in patterns:
                ...
                if found_date_str:
                    ...
                    if new_date and new_date != job.get('posted_date'):
                        ...
                        print(f"[{folder}] Updated {job['id']}: {job.get('posted_date')} -> {new_date} (found: {found_date_str})")
